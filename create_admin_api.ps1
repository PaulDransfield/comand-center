...
'@
$dir = Split-Path -Parent (Join-Path (Get-Location) "app\api\admin\route.ts")
New-Item -ItemType Directory -Force -Path $dir | Out-Null
[System.IO.File]::WriteAllText(
  (Join-Path (Get-Location) "app\api\admin\route.ts"),
  $content,
  [System.Text.UTF8Encoding]::new($false)
)
Write-Host "create_admin_api.ps1 written"
