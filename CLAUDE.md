...
> Last updated: 2026-06-02 | Session 24 — full prep + order pipeline (M116/M117/M118), modal editing surface, PostgREST dual-FK bug fix, then a structured audit pass closing H1/H2/H3/M1–M8/L5/L8 from the 2026-06-01 architecture review; Phase D watch still active until 2026-06-07
> See ARCHITECTURE-PLAN.md for the full audit + phased roadmap.

> **Session 24 invariants (2026-06-02, prep + order pipeline + audit):**
>   • **`/inventory/recipes/prep` is the manual-covers prep list.** Engine in `lib/inventory/prep-list.ts` — `aggregatePrepRequirements(items, recipeIndex, recipeNames)` mirrors `computeRecipeCost`'s recursion + ancestor cycle-guard + M111 yield math but accumulates QUANTITY instead of money. Reuses `loadRecipeIndex`. Honest-incomplete: yield-less subs flag `'sub_no_yield'` and DON'T roll up their leaf ingredients — wrong totals are never shown. Verified end-to-end via `scripts/test-prep-list-engine.ts` (Pinsa Red Sauce aggregates to 1.23 kg across Margherita + Pinsa Parma in a single SHARED line).
>   • **M116 — `prep_sessions` + `prep_session_lines`**. One row per prep run; `inputs` JSONB preserves dishes×portions. Lines FROZEN at save time (qty snapshots survive recipe edits mid-service), TEXT (method/notes/uses) read live so owner edits flow through. Partial unique index `prep_sessions_one_active_idx` enforces one active session per business; H1 race handler catches 23505 on the POST and returns the same friendly 409 shape with `note: "Concurrent create; another session won the race."`.
>   • **M117 — `recipes.portions_per_cover NUMERIC (0–10)`**. Per-dish mix share for covers auto-fill. 0.15 = "15% of guests order this dish." CHECK caps at 10 so "typed 15 thinking percent" doesn't predict 3,000 portions. NULL = no share; dish skips auto-fill.
>   • **M118 — `prep_pre_orders`**. Advance customer commitments per business/service_date/party. `items` JSONB = `[{recipe_id, qty}]`. Apply math: `freeCovers = max(0, covers − sum(party_sizes))`; share-driven qty per dish on freeCovers; pre-order items added on top. Soft delete via `archived_at`; party_size capped 1..500 (L8).
>   • **`/inventory/orders` is the smart shopping list.** `POST /api/inventory/orders/build` aggregates frozen `prep_session_lines` (kind=product) for selected sessions PLUS pre-orders in date range (run through `aggregatePrepRequirements`). Owner-mandated constraints: NEEDED qty is a read-only guide; ORDER qty input empty by default (chef writes loose text "2 jars" / "10 kg"); no stock subtraction (Phase 2 work); per-supplier "Copy for chat/email" packs filled lines into plain text. Latest-supplier lookup batches 50 product_aliases × 200 lines (M2) so it doesn't miss products at scale. products + product_aliases queries run in parallel via Promise.all (M3). `dropped_session_ids` returned for transparency on cross-tenant filter (M7).
>   • **`lib/inventory/sub-recipe-yield.ts` is the shared yield-resolution helper (M1).** `resolveSubRecipeYield({adjustedQty, recipeUnit, subPortions, yieldAmount, yieldUnit})` returns discriminated union `'portions' | 'mass_or_volume' | 'unit_mismatch' | 'no_yield'` carrying portionMultiplier + qtyInDisplayUnit + displayUnit. Prep engine calls it; cost engine will migrate when next touched. Future M111-style changes land in ONE place.
>   • **The modal pattern: tap row body → modal; checkbox column toggles check.** Modal works in BOTH prep mode (session line tap) and create mode (preview row tap) via unified `openModal: { line: PrepSessionLine, session_line_id: string | null }` state. `session_line_id` non-null = prep mode, save updates `sessionLines`; null = preview mode, save triggers `scheduleRecompute()` (debounced 400ms, M4). Method editor falls back from `recipes.method` to `recipes.notes` (legacy/bulk-importer writes). H3: archived sub-recipes show a coral banner. M5: `previewToSessionLine` converters drop `as any` since meta shapes align.
>   • **`feedback_postgrest_dual_fk_ambiguity.md` is load-bearing for ANY future query that embeds across a child with multiple FKs to the same parent.** `recipe_ingredients` has both `recipe_id` AND `subrecipe_id` pointing at `recipes`; PostgREST's `recipes!inner(...)` auto-detects + can pick the wrong FK, silently dropping rows. Use the two-step pattern (fetch parent ids first, filter child by `.in('recipe_id', [...])`) for ALL such cases. Fixed in both the session GET + preview POST endpoints.
>   • **Frozen-vs-live discipline.** Quantities = frozen at session save (`prep_session_lines.total_qty`). Text (method, notes, uses, archived_at) = read live on every GET. Owner mid-service edits to `recipes.method` / `recipe_ingredients.notes` surface immediately on next refresh; qtys stay stable because the engine's intent at save time is what the kitchen committed to.
>   • **Text input caps everywhere (H2 + M8).** `recipes.method` 20k chars (client maxLength + counter when <500 remaining). `recipes.notes` already capped at 2k via PATCH. `prep_sessions.name` / `prep_pre_orders.party_name` 200 chars. `prep_pre_orders.notes` 2000 chars. `recipe_ingredients.notes` 500 chars. `recipe_ingredients.unit` 32 chars. All server-side; client can't bypass.
>   • **Toggle rollback uses pre-optimistic snapshot (M6).** Captures the actual current value inside `setSessionLines` callback before applying the optimistic patch, rolls back to that on error instead of to the stale `line` arg.
>   • **BOOKING-API-COVERS-PLAN.md parks the eventual SuperbExperience / OpenTable / ResDiary / Caspeco-bookings integration.** When owner picks a provider, the seam is: covers number sourced externally, fed into the same Apply math. Schema additions sketched. Don't build proactively.
>   • **POS-RECIPE-MAPPING-PLAN.md / M097 `pos_menu_items.recipe_id` is the demand-prediction reservoir.** Owner mapping or POS connector fills it; then predicted covers can drive auto-fill from historical sales. Parked.
>   • **Phase D watch STILL active until 2026-06-07** (carried over from Session 22). Vero `needs_review_agreement_pct` vs 46.8% baseline + queue depth trend + normalization re-confirm rate. Ticket 2 voucher cache re-warm still parked.
>
