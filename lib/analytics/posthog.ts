...
//
// PostHog event tracking — records user behavior for analytics.
...
//
// Events we track (privacy-safe — no personal data in event properties):
//   - page_view
//   - signup, login, logout
...
function getPostHog() {
  ...
  import('posthog-js').then(({ default: posthog }) => {
    ...
    // PostHog not installed — npm install posthog-js
...
export function track(event: EventName, properties?: EventProperties) {
  const ph = getPostHog()
  if (!ph) return  // analytics not configured — fail silently
 ...
 * Call this after login. Never pass email — use the UUID only.
 */
export function identify(userId: string, plan: string) {
  ...
  try {
    ph.identify(userId, {
      plan,
      // No email, no name — UUID only for GDPR compliance
    })
  } catch {}
}
