...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (data.session) {
      // Session established â€” do a full page reload to /dashboard
    ...
    <div style={{ minHeight:'100vh', background:'var(--parchment)', display:'flex', alignItems:'center', justifyContent:'center', padding:'20px' }}>
      <div style={{ background:'var(--white)', border:'1px solid var(--border)', borderRadius:'16px', padding:'36px', width:'400px', maxWidth:'100%', boxShadow:'0 4px 24px rgba(0,0,0,.08)' }}>
        ...
        <p style={{ fontSize:'13px', color:'var(--ink-3)', marginBottom:'24px' }}>
          {mode === 'login' ? 'Sign in to your restaurant command center' : mode === 'signup' ? '30-day free trial Â· No credit card required' : "We'll send a reset link to your email"}
        </p>

        {message && <div style={{ background:'var(--green-lt)', border:'1px solid var(--green-mid)', borderRadius:'8px', padding:'10px 14px', fontSize:'13px', color:'var(--green)', marginBottom:'16px' }}>âœ“ {message}</div>}
        ...
        <form onSubmit={mode === 'login' ? handleLogin : mode === 'signup' ? handleSignup : handleForgot}>
          ...
          {mode !== 'forgot' && (
            <div style={{ marginBottom:'14px' }}>
              ...
              <input className="input" type="password" placeholder={mode === 'signup' ? 'At least 8 characters' : 'â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢'} value={password} onChange={e => setPassword(e.target.value)} required minLength={mode === 'signup' ? 8 : 1} autoComplete={mode === 'signup' ? 'new-password' : 'current-password'} />
            </div>
          )}
          {mode === 'login' && (
          ...
          <button type="submit" className="btn btn-primary btn-full" disabled={loading} style={{ marginTop:'4px' }}>
            {loading ? <><span className="spin">âŸ³</span> Please waitâ€¦</> : mode === 'login' ? 'Sign in' : mode === 'signup' ? 'Create account' : 'Send reset link'}
          </button>
        </form>
        ...
        <div style={{ textAlign:'center', marginTop:'20px', fontSize:'13px', color:'var(--ink-3)' }}>
          ...
          : (<button style={{ background:'none', border:'none', cursor:'pointer', color:'var(--blue)', fontSize:'13px', fontFamily:'var(--font)', padding:'0', textDecoration:'underline' }} onClick={() => { setMode('login'); setError(''); setMessage('') }}>â† Back to login</button>)}
        </div>
