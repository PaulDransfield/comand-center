...
// );
// -- No RLS needed — only accessible via service role key (admin only)
...
export async function POST(req: NextRequest) {
  ...
  // Send Slack notification (fire-and-forget — don't block the response)
...
async function notifySlack(data: {
  ...
  await fetch(webhookUrl, {
    ...
    body:    JSON.stringify({
      text: `🎉 New beta signup: ${data.name} from ${data.restaurant_name}`,
      blocks: [
        { type: 'header', text: { type: 'plain_text', text: `🎉 New beta application` }},
        { type: 'section', fields: [
