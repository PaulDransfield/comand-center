#!/usr/bin/env node
// scripts/flip-vero-v2-budgeting-flag.mjs
//
// Activates PREDICTION_V2_BUDGETING for Vero Italiano. Once flipped, the
// next runSync() for her business will write consolidated_monthly_v1.0
// rows to the `forecasts` table instead of the legacy rolling-avg ×
// hardcoded-seasonal-factor math.
...
const FLAG     = 'PREDICTION_V2_BUDGETING'
...
const upsert = await fetch(`${URL}/rest/v1/business_feature_flags`, {
  ...
  body: JSON.stringify({ org_id: VERO_ORG, business_id: VERO_BIZ, flag: FLAG, enabled: true, updated_at: new Date().toISOString() }),
})
const result = await upsert.text()
if (!upsert.ok) { console.error(`Upsert failed: ${upsert.status} ${result}`); process.exit(1) }
...
if (after[0]?.enabled === true) {
  console.log('\n✓ PREDICTION_V2_BUDGETING is now ENABLED for Vero Italiano')
  console.log('  Next sync writes consolidated_monthly_v1.0 rows to forecasts table.')
} else {
