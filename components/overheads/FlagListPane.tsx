  ...
  return (
    <div
      ...
      style={{
        ...
        borderLeft:  `3px solid ${selected ? UXP.greenDeep : 'transparent'}`,
        opacity:     isResolved ? 0.55 : 1,
      }}
    >
      <div style={{ minWidth: 0 }}>
        <div style={{
          fontSize: 13, fontWeight: 600, color: UXP.ink1,
        ...
        <div style={{ display: 'flex', gap: 4, alignItems: 'center', flexWrap: 'wrap' as const }}>
          ...
          <span style={{ fontSize: 10, color: UXP.ink4 }}>{periodLabel}</span>
          <span style={{
            fontSize: 9, color: UXP.ink4, marginLeft: 'auto',
            textTransform: 'uppercase' as const, letterSpacing: '0.05em', fontWeight: 500,
            padding: '1px 6px', background: UXP.subtleBg,
            border: `1px solid ${UXP.borderSoft}`, borderRadius: 3,
          }}>
      ...
      <div style={{ textAlign: 'right' as const }}>
        <div style={{
          fontSize: 14, fontWeight: 700, color: UXP.ink1, lineHeight: 1,
          whiteSpace: 'nowrap' as const, fontVariantNumeric: 'tabular-nums' as const,
        }}>
          {fmtKr(f.amount_sek)}
        </div>
        <div style={{ fontSize: 10, color: UXP.ink4, marginTop: 3 }}>{t('perMo')}</div>
        {periodCount > 1 && (
          <div style={{ fontSize: 9, color: UXP.ink4, marginTop: 3, fontWeight: 500 }}>
...
function Pill({ children, active, onClick, tone }: {
  ...
  const accentInk =
    ...
    tone === 'info'   ? UXP.lavDeep :
    tone === 'purple' ? '#6b21a8' : UXP.ink2
  return (
    <button
      type="button"
      onClick={onClick}
      style={{
        background:   active ? UXP.ink1 : '#e9eae5',
        border:       'none',
        padding:      '4px 10px',
        borderRadius: 999,
        fontSize:     11,
...
function resolvedToggleStyle(active: boolean): React.CSSProperties {
  return {
    background:   active ? UXP.ink1 : '#f3f4f0',
