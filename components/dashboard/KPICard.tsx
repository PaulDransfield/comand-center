...
//
// A KPI CARD — shows one financial metric.
// Used in the dashboard for: Revenue, Margin, Staff %, Food %, Rent %, Profit.
//
// Props:
//   label       — "Staff Cost"
//   value       — formatted value to display: "224 978 kr" or "44.5%"
//   delta       — change vs last month: "+9.6%" (green if positive, red if negative)
//   target      — the target percentage (e.g. 40 for staff cost)
//   actual      — the actual percentage (e.g. 44.5)
//   overIsGood  — false for cost metrics (over target = red), true for revenue/margin
//   sub         — small subtitle text: "of revenue · target 40%"
  ...
  if (target !== undefined && actual !== undefined) {
    ...
    if (isGood) {
      ...
      statusText = overIsGood
        ? (overTarget ? '↑ above target' : '↓ below target')
...
// Usage meter — shows tokens, documents etc as a simple bar
    ...
    <div style={{ ...S.card, padding: '10px 14px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
        ...
        <span style={{ fontSize: 11, fontFamily: 'var(--mono)', color: atLimit ? 'var(--red)' : 'var(--ink-4)' }}>
          {used.toLocaleString()}{unit} / {limit === Infinity ? '∞' : limit.toLocaleString()}{unit}
        </span>
      </div>
      <div style={S.barTrack}>
        <div style={{
      ...
      {atLimit && (
        <div style={{ ...S.sub, color: 'var(--red)', marginTop: 4 }}>Limit reached — upgrade to continue</div>
      )}
    </div>
  )
}
