...
export async function GET(req: NextRequest) {
  ...
  // ── 3. Group errors by provider and alert if threshold exceeded ─
...
async function checkIntegration(integ: {
  ...
  if (integ.token_expires_at) {
    ...
    if (daysLeft <= 0) {
      return { status: 'error', responseTimeMs: 0, errorCode: 'TOKEN_EXPIRED', errorMessage: 'Token expired — re-authorisation required', tokenDaysLeft: 0 }
    }
    if (daysLeft <= 7) {
      // Token expires soon — warn but don't error
...
async function sendSlackAlert(provider: string, count: number, level: 'error' | 'critical') {
  ...
  try {
    await fetch(webhookUrl, {
      ...
      body:    JSON.stringify({
        text:        `${emoji} CommandCenter Alert — ${provider} integration issue`,
        attachments: [{
          color: colour,
          blocks: [
