...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) {
        // No session â€” link expired or already used
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Password updated â€” sign out all other sessions and go to dashboard
      ...
      <div style={{ background:'var(--white)', border:'1px solid var(--border)', borderRadius:16, padding:36, width:400, maxWidth:'100%', boxShadow:'0 4px 24px rgba(0,0,0,.08)' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display:'flex',flexDirection:'column',gap:14 }}>
          ...
          <button type="submit" className="btn btn-primary btn-full" disabled={loading} style={{ marginTop:4 }}>
            {loading ? <><span className="spin">âŸ³</span> Updatingâ€¦</> : 'Update password'}
          </button>
        </form>
