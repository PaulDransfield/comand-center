...
'@
$dir = Split-Path -Parent (Join-Path (Get-Location) "app\api\invoices\extract\route.ts")
New-Item -ItemType Directory -Force -Path $dir | Out-Null
[System.IO.File]::WriteAllText(
  (Join-Path (Get-Location) "app\api\invoices\extract\route.ts"),
  $content,
  [System.Text.Encoding]::UTF8
)
Write-Host "create_invoice_extract.ps1 written"
