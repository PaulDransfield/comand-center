...
export default function DashboardPage() {
  ...
  if (loading) return <div style={{ padding: 40, color: '#666' }}>Loadingâ€¦</div>
      ...
      {showAdd && (
        <>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', background: 'white', borderRadius: 14, padding: 28, width: 440, maxWidth: '94vw', zIndex: 101, boxShadow: '0 20px 60px rgba(0,0,0,.2)' }}>
            ...
            <p style={{ fontSize: 13, color: '#999', marginBottom: 20 }}>Each restaurant, bar, or cafÃ© is a separate location.</p>
        ...
        <>
          <div style={{ marginBottom: 24 }}>
            ...
            <p style={{ color: '#999', fontSize: 12, marginTop: 4 }}>
              {biz.type ?? 'Restaurant'}{biz.city ? ` Â· ${biz.city}` : ''} Â· {new Date().toLocaleDateString('sv-SE', { month: 'long', year: 'numeric' })}
            </p>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 20, flexWrap: 'wrap' }}>
            {[
              { label: 'ðŸ“š Notebook',     href: '/notebook'     },
              { label: 'ðŸ“Š Tracker',      href: '/tracker'      },
              { label: 'âŸ³ Integrations', href: '/integrations' },
              { label: 'â†‘ Upgrade',       href: '/upgrade'      },
            ].map(l => (
