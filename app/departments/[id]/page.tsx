...
function getWeekBounds(offset = 0) {
  ...
  return { from: mon.toISOString().slice(0,10), to: sun.toISOString().slice(0,10), weekNum: getISOWeek(mon), label: mM === sM ? `${mon.getDate()}–${sun.getDate()} ${mM}` : `${mon.getDate()} ${mM} – ${sun.getDate()} ${sM}`, mon }
}
...
export default function DepartmentDetailPage() {
  ...
  const chartDays = Array.from({ length: dayCount }, (_, i) => {
    ...
    return { dateStr: ds, revenue: row?.revenue ?? 0, covers: row?.covers ?? 0, dayName: viewMode === 'week' ? DAYS[dayIdx] : String(i + 1), isToday: ds === now.toISOString().slice(0, 10), isFuture: d > now, dayIdx }
  })
