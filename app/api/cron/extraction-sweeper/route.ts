...
export async function GET(req: NextRequest) {
  ...
  if (resetErr) console.warn('[sweeper] reset RPC failed:', resetErr.message)
  ...
  // Fire one worker per ready job. The worker's claim RPC is idempotent
  // (skip-locked) so duplicate fires land on different jobs or no-op.
